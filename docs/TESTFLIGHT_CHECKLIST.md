...
### C-3. TestFlight で自分の端末に配る（内部テスト）

ビルドが成功すると、ipa は自動で App Store Connect にアップロードされる。
**自分の実機で試すだけなら「内部テスト」を使う。ベータ審査は不要で、すぐ配布できる。**

1. App Store Connect → ラキカラ → **TestFlight** タブ
2. ビルド（`1.0.0 (1)` など）が並ぶ。「処理中」が消えるまで数分〜30分待つ
3. 左側の **「内部テスト」** → **＋** でグループを作成（名前は任意）
4. そのグループに**自分を追加**する
5. iPhone に **TestFlight** アプリを入れ、同じ Apple ID でサインイン

> **外部テスト**（最大10,000人に配る）はベータ審査が必要で、
> App Store Connect 側に **Beta App Description（テスト情報）** の入力が必須。
> 未入力のまま自動提出すると `422 Beta App Description is missing` で失敗する。
> そのため `codemagic.yaml` の `submit_to_testflight` は既定で `false` にしてある。
> 外部テスターに配る段階（Step 9）で、説明文を入れてから `true` にする。
> **アップロード自体はこの設定に関係なく行われる。**
